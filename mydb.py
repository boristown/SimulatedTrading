...
def get_markets_from_endtags(endtags, mycursor):
    markets = []
    select_tags_statment = "select tags.* from tags inner join predictlog on tags.symbol = predictlog.symbol and  predictlog.PREDICTDATE > '1950-1-1' where tag in (%s) " % ','.join(['%s']*len(endtags))
